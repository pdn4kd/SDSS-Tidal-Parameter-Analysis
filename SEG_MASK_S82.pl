			...
			foreach my $sexCount (0 .. scalar @N - 1) {
				...
				if ($x_1 >= $X[$sexCount]  && $y_1 >= $Y[$sexCount] && $y_2 <= $Y[$sexCount] && $x_2 <= $X[$sexCount]) {
					...
					if ($N[$sexCount] != 1) { #we have to jump through some hoops for the mask_1b to work.
						print $Maker "imreplace p${ID[$posCount]}_S82.mask_1b.fits value =9999 lower=1 upper=1\n"; #change extraneous bright source to something excessive that will become 0.
					...
					print $Maker "imcopy p${ID[$posCount]}_S82.mask_1b.fits FMASK_b.p${ID[$posCount]}_S82.fits\n";
