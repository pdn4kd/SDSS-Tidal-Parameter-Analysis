...
foreach my $galaxy_fits (@galaxy_fits) { #iterate over all fit types
	if (-e "p$nyuID[$galCount]_DR7.model_$galaxy_fits.fits") { #if the galaxy was successfully fit
...
foreach my $posCount (0 .. scalar @N - 1)
{
	...
	open my $mgalfitaa, '>', "mp$nyuID[$galCount]_DR7.galfit_aa" or die "cannot open mp$nyuID[$galCount]_DR7.galfit_aa $!"; #GALFIT 2 component free Galaxy model (n=anything disk, n=anything bulge)
...
F) FMASK.p$nyuID[$galCount]_DR7.fits      # Bad pixel mask (FITS image or ASCII coord list)
...
================================================================================
# IMAGE and GALFIT CONTROL PARAMETERS
A) bmodel.p$nyuID[$galCount]_DR7_aa.fits      # Input data image (FITS file)
B) mp$nyuID[$galCount]_DR7.model_aa.fits      # Output data image block
C) #                   # Sigma image name (made from data if blank or "none") 
D) scpsf.$nyuID[$galCount]_DR7.fits           # Input PSF image and (optional) diffusion kernel
E) 1                  # PSF fine sampling factor relative to data 
F) FMASK.p$nyuID[$galCount]_DR7.fits      # Bad pixel mask (FITS image or ASCII coord list)
