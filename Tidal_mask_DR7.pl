...
open my $SEX, '<', "result_DR7.csv" or die "cannot open result.csv: $!";
...
print "${nyuID[$posCount]}_DR7.fits\n";

#NEW MASK USING SEXTRACTOR
open my $inPositions, '<', "tidal_mask.${nyuID[$posCount]}_DR7.csv" or die "cannot open tidal_mask.${nyuID[$posCount]}_DR7.csv: $!";
...
$ellipse->wfits("tmask_1a.p${nyuID[$posCount]}_DR7.fits");  
$ellipse_2->wfits("tmask_1b.p${nyuID[$posCount]}_DR7.fits"); 

my $Mimage = rfits("tmask_1b.p${nyuID[$posCount]}_DR7.fits");
...
$Un_masked->wfits("tmask.p${nyuID[$posCount]}_DR7.fits");
