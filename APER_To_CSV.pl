...
while (my $file = readdir $fdDir)
	{
	chomp $file; 
	next unless $file =~ m/^nyu(.+)\.aper\.cat$/; # read only files that began with m/^nyu\ and end with .csv$/ #SDSS_STARS

	$objectName = $1;
	print "nyu$objectName.aper.csv\n";
...
open(IN,"<nyu$objectName.aper.cat");

open(OUT,">nyu$objectName.aper.csv");
