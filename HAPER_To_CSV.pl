...
while (my $file = readdir $fdDir)
	{
	chomp $file; 
	next unless $file =~ m/^hpnyu(.+)\.aper\.cat$/; # read only files that began with m/^hpnyu\ and end with .csv$/ #SDSS_STARS
	

	$objectName = $1;
	print "hpnyu$objectName.csv\n";
...
open(IN,"<hpnyu$objectName.aper.cat");

open(OUT,">hpnyu$objectName.aper.csv");
