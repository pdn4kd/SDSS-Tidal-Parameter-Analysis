...
open my $SEX, '<', "result_DR7.csv" or die "cannot open result_DR7.csv: $!"; #Opens the main result.csv that contains all the key parameters
...
open my $DELMAG, '>', "Parameters_DR7.csv" or die "cannot open DEL_MAG.csv: $!"; #Magnitude comparison file
...
open my $COLDMASK, '>', "COLDMASK.p${nyuID[$posCount]}_DR7.csv" or die "cannot open COLDMASK.p${nyuID[$posCount]}_DR7.galfit_input.csv: $!"; #COLDMASK
print $COLDMASK "NUMBER,X_IMAGE,Y_IMAGE,A_IMAGE,B_IMAGE,THETA_IMAGE,KRON_RADIUS\n";

open my $HOTMASK, '>', "HOTMASK.p${nyuID[$posCount]}_DR7.csv" or die "cannot open p${nyuID[$posCount]}_DR7.galfit_input.csv: $!"; #HOTMASK
print $HOTMASK "NUMBER,X_IMAGE,Y_IMAGE,A_IMAGE,B_IMAGE,THETA_IMAGE,KRON_RADIUS\n";

open my $Tp_mask, '>', "tidal_mask.${nyuID[$posCount]}_DR7.csv" or die "cannot open tidal_mask.${nyuID[$posCount]}_DR7.galfit_input.csv: $!"; #Tidal parameter mask
...
##########################
	...
	open my $ALLSEX, '<', "${nyuID[$posCount]}_DR7.aper.csv" or die "cannot open ${nyuID[$posCount]}_DR7.aper.csv: $!"; #Opens the main all aper.csv that contains all the key parameters
	
	print " This is image ${nyuID[$posCount]}_DR7.fits\n ";
...
#####################
#COLD SEX
				open my $COLDSEX, '<', "p${nyuID[$posCount]}_DR7.aper.csv" or die "cannot open p${nyuID[$posCount]}_DR7.aper.csv: $!"; #Opens the main cold postage.csv that contains all the key parameters
				print "\n";
				print "Cold SEX p${nyuID[$posCount]}_DR7.fits","\n";
...
#				print join(',',@dim),"\n";
				...
				{ 
			   			...
			   			) 
					{
								...
								print $DELMAG "$nyuID[$posCount]_DR7,$gN[$ColdCount]c,$MAG_AUTO[$ColdCount],$MAGERR_AUTO[$ColdCount],$petro_mag[$posCount],$petroErr_mag[$posCount],$model_mag[$posCount],$modelErr_mag[$posCount],$gX[$ColdCount],$gY[$ColdCount],$Re[$posCount],$conc_r[$posCount]\n"; #header for delmag																				
...
####################
###HOTSEX
				open my $HOTSEX, '<', "hp${nyuID[$posCount]}_DR7.aper.csv" or die "cannot open hp${nyuID[$posCount]}_DR7.aper.csv: $!"; #Opens the main HOT postage.csv that contains all the key parameters
				print "\n";
				print "HOT SEX SOURCE p${nyuID[$posCount]}_DR7.fits","\n";
