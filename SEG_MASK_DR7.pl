		...
		#	FOR ALL THE CRAP!
			foreach my $sexCount (0 .. scalar @N - 1) {
				...
				if ($x_1 >= $X[$sexCount]  && $y_1 >= $Y[$sexCount] && $y_2 <= $Y[$sexCount] && $x_2 <= $X[$sexCount]) {
					...
					open my $GALFIT_input, '>', "p${ID[$posCount]}_DR7.galfit_input.csv" or die "cannot open p${ID[$posCount]}_DR7.galfit_input.csv: $!";
					...
					#Tidal mask CSV seed
					open my $Tp_mask, '>', "tidal_mask.${ID[$posCount]}_DR7.csv" or die "cannot open tidal_mask.${ID[$posCount]}_DR7.galfit_input.csv: $!"; #Tidal parameter mask
