...
while (my $file = readdir $fdDir)
	{
	chomp $file; 
	next unless $file =~ m/^pnyu(.+)\.aper\.cat$/; # read only files that began with m/^pnyu\ and end with .csv$/ #SDSS_STARS
	

	$objectName = $1;
	print "pnyu$objectName.aper.csv\n";
...
open(IN,"<pnyu$objectName.aper.cat");

open(OUT,">pnyu$objectName.aper.csv");
