...
{
	open my $instars, '<', "$nyuID[$posCount]_S82.aper.csv" or die "cannot open $nyuID[$posCount]_S82.aper.csv $!";
	...
	#output.
		...
		print "The parent galaxy is located at $Xp,$Yp\n";
				...
				$Yg_p_cutmax = sprintf("%.0f",$Yp + $Xp_cut);
					...
					{
					print $coutouts "imcopy $nyuID[$posCount]_S82.fits[$Xg_p_cutmin:$Xg_p_cutmax,$Yg_p_cutmin:$Yg_p_cutmax] p$nyuID[$posCount]_S82.fits\n";
					print "imcopy $nyuID[$posCount]_S82.fits[$Xg_p_cutmin:$Xg_p_cutmax,$Yg_p_cutmin:$Yg_p_cutmax] p$nyuID[$posCount]_S82.fits\n";					
					}
				}
				else
				{
