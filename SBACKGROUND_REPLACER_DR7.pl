...
#This script finds the typical background (sky) light values for the image
...
foreach my $posCount (0 .. scalar @nyuID - 1) {
		if ((-e "p${nyuID[$posCount]}_DR7.fits") && (-e "p${nyuID[$posCount]}_DR7.seg.csv")) {
		print "p${nyuID[$posCount]}_DR7.seg.csv","\n";
		open my $inPositions_1, '<', "p${nyuID[$posCount]}_DR7.seg.csv" or die "cannot open p${nyuID[$posCount]}_DR7.seg.csv: $!"; #object locations to avoid
		...
		my $image = rfits("Good.p${nyuID[$posCount]}_DR7.fits"); 
		...
		$ellipse->wfits("Full_mask.p${nyuID[$posCount]}_DR7_1a.fits");  

		$ellipse_2->sethdr($image->hdr);
		$ellipse_2->wfits("Full_mask.p${nyuID[$posCount]}_DR7_1b.fits"); 

		#displ masked image
		my $Mimage = rfits("Full_mask.p${nyuID[$posCount]}_DR7_1b.fits");
		...
		$Full_masked_image->wfits("Full_masked.p${nyuID[$posCount]}_DR7.fits");
		...
		$Good_values-> wfits("background.p${nyuID[$posCount]}_DR7.fits");
