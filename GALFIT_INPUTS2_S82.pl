...
open my $inGalPositions, '<', "result_S82.csv" or die "cannot open result.csv: $!"; #Change the input to your input file with the galaxy coordinates
...
open my $mgalfit_batch, '>', "GALFIT_MBATCH_S82.sh" or die "cannot open GALFIT_BATCH.sh: $!";
...
foreach my $galaxy_fits (@galaxy_fits) { #iterate over all fit types
	if (-e "p$nyuID[$galCount]_S82.model_$galaxy_fits.fits") { #if the galaxy was successfully fit
