...
open my $paper_all_S82, '>', "paper_all_S82.sh" or die "cannot open paper_all_S82.sh $!";
open my $hot_S82, '>', "haper_S82.sh" or die "cannot open haper_S82.sh $!";
...
{
	open my $SEXaper_r, '>', "p$nyuID[$posCount]_S82.aperture.sex" or die "cannot open p$nyuID[$posCount].aperture.sex $!";

print $SEXaper_r <<___end___;
#-------------------------------- Catalog ------------------------------------
 
CATALOG_NAME     p$nyuID[$posCount]_S82.aper.cat  # name of the output catalog
...
CHECKIMAGE_TYPE  APERTURES   # can be NONE, BACKGROUND, BACKGROUND_RMS,
                                ...
                                # or APERTURES
CHECKIMAGE_NAME  p$nyuID[$posCount]_S82.aper.fits     # Filename for the check-image
...
open my $hot_aper_r, '>', "hp$nyuID[$posCount]_S82.aperture.sex" or die "cannot open hp$nyuID[$posCount].aperture.sex $!";

print $hot_aper_r <<___end___;
#-------------------------------- Catalog ------------------------------------
 
CATALOG_NAME     hp$nyuID[$posCount]_S82.aper.cat  # name of the output catalog
CATALOG_TYPE     ASCII_HEAD     # NONE,ASCII,ASCII_HEAD, ASCII_SKYCAT,
                                # ASCII_VOTABLE, FITS_1.0 or FITS_LDAC
PARAMETERS_NAME  test1.param  # name of the file containing catalog contents
...
CHECKIMAGE_TYPE  APERTURES   # can be NONE, BACKGROUND, BACKGROUND_RMS,
                                ...
                                # FILTERED, OBJECTS, -OBJECTS, SEGMENTATION,
                                # or APERTURES
CHECKIMAGE_NAME  hp$nyuID[$posCount]_S82.aper.fits     # Filename for the check-image
...
print $paper_all_S82 "sex p$nyuID[$posCount]_S82.fits -c p$nyuID[$posCount]_S82.aperture.sex \n";
#system("/usr/local/bin/sex /$dir/p$nyuID[$posCount]_S82.fits -c /$dir/p$nyuID[$posCount]_S82.aperture.sex");
print $hot_S82 "sex p$nyuID[$posCount]_S82.fits -c hp$nyuID[$posCount]_S82.aperture.sex \n";
print "sex p$nyuID[$posCount]_S82.fits -c hp$nyuID[$posCount]_S82.aperture.sex \n";
#system("/usr/local/bin/sex /$dir/p$nyuID[$posCount]_S82.fits -c /$dir/hp$nyuID[$posCount]_S82.aperture.sex");
}
